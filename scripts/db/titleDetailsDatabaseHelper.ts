...
// Return a single Title by number and slug (or undefined). Accepts number or string.
export async function getTitleByNumberAndSlug(
  titleNumber: number,
  slug: string
): Promise<Title | undefined> {
  console.log("getTitleByNumberAndSlug: called with", titleNumber);
  if (titleNumber == null || slug == null) return undefined;
  const db = await readDb();
  // Use lowdb find to avoid pulling the full array into memory for mutation
  return db
    .get(TITLE_DETAILS_KEY)
    .find({ number: titleNumber, agencySlug: slug })
    .value();
