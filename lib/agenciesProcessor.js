...
const titlesDb = require("../scripts/db/titleDatabaseHelper");
...
async function init(app, db, options = {}) {
  ...
  app.get("/api/agencies", async (req, res) => {
    try {
      ...
      const titles = (await titlesDb.getTitles()) || [];
  ...
  app.get("/api/status", async (req, res) => {
    const lastUpdated = await agencyDb.getLastUpdated();
    const agencies = (await agencyDb.getAgencies()) || [];
    ...
    try {
      ...
      try {
        if (!searchProcessor || !searchProcessor.handleSearch) {
          throw new Error("searchProcessor not provided or missing handleSearch method");
        }
        const json = await searchProcessor.handleSearch(db, slug, page, forceRefresh);
        return res.json(json);
      } catch (err) {
        if (err && err.status) {
          return res.status(502).json({ error: "upstream fetch failed", status: err.status, body: err.body });
        }
        throw err;
      }
    } catch (err) {
